/**
 * ULRIC-X MD v13.2.1 - Enterprise WhatsApp Multi-User Connection Manager
 *
 * STABILITY FIXES:
 * 1. SHARED STATE OBJECT: All event handlers reference a single `state` object.
 *    This fixes the closure bug where 515 retries would fail because `pairCode`
 *    and `everConnected` were snapshots, not references.
 * 2. ROBUST 515 HANDLING: When 515 occurs, the socket is recreated with the
 *    SAME session creds. The new socket properly binds ALL handlers and updates
 *    the shared state. This prevents the "Logging In..." stuck issue.
 * 3. EXPONENTIAL BACKOFF: Reconnects use exponential backoff (5s, 10s, 20s, 40s)
 *    to prevent aggressive reconnect loops.
 * 4. CRASH PROTECTION: Robust error handling prevents the bot from crashing
 *    on socket errors, stream errors, or network interruptions.
 * 5. DASHBOARD SYNC: The `connections` Map is immediately updated on `open`
 *    and `close` events so the dashboard always shows the correct status.
 */
const fs = require('fs');
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
        ...
        if (statusCode === 515 && state.retry515Count < 5) {
          ...
          setTimeout(async () => {
            try {
              ...
              // ATTACH HANDLERS TO RETRY SOCKET
      ...
      if (state.everConnected) {
        const result = session.recordReconnectFailure(jid, 5); // Increased limit to 5
        if (result.deleted) {
